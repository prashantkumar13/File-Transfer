...
const FilePicker = ({
  ...
  const [buttonStatus, setButtonStatus] = useState(0);  // 0=upload button 1 = generating icon 2=download button
  const {user}=useUser();
  ...
  return (
    <div className="container p-5  border border-black dark:border-white  border-dashed rounded-lg">
    <div className=""> <input type="text" onChange={onInputChange} className="p-2 border rounded-md w-full bg-transparent text-black dark:text-white" placeholder="Enter the title for the Document" /></div>
      <div className="grid grid-cols-12 h-full gap-x-5 justify-between p-2 ">
        <Files
          ...
          minFileSize={minFileSize * 1000000}
          clickable
        >
          <div className="w-full rounded-lg p-10 border-2 border-gray-200/20 border-dashed bg-zinc-500/10">
            ...
            <div className="text-center text-sm">
              <p className="font-semibold text-2xl text-black dark:text-white">Drag your files here</p>
              <p className="text-gray-400 mt-3">Drag and drop your files: {formatList} here.</p>
              <p className="text-gray-400">Maximum size: {maxFileSize}MB</p>
              <div className="text--caption my-2 text-gray-400">- OR -</div>
        ...
        <div className="md:col-span-6 pt-10 pb-10 col-span-12 w-full h-full" >
          <span className="mb-3 flex justify-between">
            <h3 className="text-lg font-semibold text-black dark:text-white">Uploaded files</h3>
            {selectedFiles.length > 0 && (
              <button
                className="underline text-xs text-black dark:text-white"
                onClick={() => {
                  setSelectedFiles([]);
                  setFilePercentages([]);
                  setButtonStatus(0)
      ...
      {(buttonStatus==0) && <Button variant="filled"  color="blue" onClick={uploadFiles}>
        Upload Files
      </Button>}

      {(buttonStatus==1) && <div className="flex justify-center items-center border border-black dark:border-white rounded-md px-4 py-1 bg-zinc-500/10">

      <Loader size={20} color="gray"/>
      <p className="p-1 font-medium text-gray-500 "> Generating Link</p>
      </div>
        }
      ...
      {/* {fileUrls.length > 0 && fileUrls.map((url, index) => (
        <div key={index}>
          <a href={url} download>
            <Button>Download File {index + 1}</Button>
          </a>
        </div>
      ))} */}
